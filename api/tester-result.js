...
export default async function handler(req,res){
  ...
  if(!url||!secret)return res.status(503).json({ok:false,error:'Evidence storage not configured'});
  ...
  try{const response=await fetch(url,{method:'POST',redirect:'follow',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({secret,kind:'auto_test',payload}),signal:AbortSignal.timeout(9000)});if(!response.ok)throw new Error(`Sheets ${response.status}`);const data=await response.json();if(data?.ok===false)throw new Error(data?.error||'Rejected');return res.status(200).json({ok:true});}catch(error){console.error('Tester result write failed',error?.message||error);return res.status(502).json({ok:false,error:'No se pudo registrar la prueba'});}
}
