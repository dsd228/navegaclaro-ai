const NC_SPREADSHEET_ID='1TwFkfaJgNMk_bLT-VGb6c2I33eJMC-HBukECnKF9Klw';
const NC_SHEETS={
...
function spreadsheet_(){return SpreadsheetApp.openById(NC_SPREADSHEET_ID);}

function ensureSheets_(){const ss=spreadsheet_();Object.keys(NC_SHEETS).forEach(name=>{let sheet=ss.getSheetByName(name);if(!sheet)sheet=ss.insertSheet(name);const headers=NC_SHEETS[name];if(sheet.getLastRow()===0)sheet.getRange(1,1,1,headers.length).setValues([headers]).setFontWeight('bold').setBackground('#11151a').setFontColor('#ffffff');sheet.setFrozenRows(1);});}
...
function append_(name,row){const lock=LockService.getScriptLock();lock.waitLock(5000);try{spreadsheet_().getSheetByName(name).appendRow(row);}finally{lock.releaseLock();}}
...
function rawRows_(name){const sheet=spreadsheet_().getSheetByName(name);if(!sheet||sheet.getLastRow()<2)return[];return sheet.getRange(2,1,sheet.getLastRow()-1,sheet.getLastColumn()).getValues();}
...
function rowsByHeader_(name){const sheet=spreadsheet_().getSheetByName(name);if(!sheet||sheet.getLastRow()<2)return[];const values=sheet.getDataRange().getValues();const headers=values.shift().map(String);return values.filter(row=>row.some(cell=>cell!==''&&cell!==null)).map(row=>{const obj={};headers.forEach((h,i)=>obj[h]=row[i]);return obj;});}
