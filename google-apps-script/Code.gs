...
function appendTest_(p){const condition=normalizeCondition_(p.condition)==='con'?'Con NC':'Sin NC';append_('TEST_USUARIOS',[safe_(p.participant,40),condition,safe_(p.task,160),Boolean(p.success),num_(p.timeSeconds),num_(p.errors),num_(p.help),Math.min(5,Math.max(1,num_(p.ease))),safe_(p.quote,350),safe_(p.notes,500),new Date()]);}
